    ...
    <>
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm">
        ...
        <div className="space-y-4">
          {(
            Object.keys(preferences.notifications) as (keyof UserPreferences["notifications"])[]
          ).map((key) => (
            <label key={key} className="flex items-center justify-between">
            <span className="text-gray-700 dark:text-gray-300 capitalize">
              {key === "email" ? "이메일 알림" : key === "push" ? "푸시 알림" : "데스크톱 알림"}
            </span>
              {/* On: bg-blue-500 */}
              ...
              onClick={() => {updateNotifications(key, !preferences.notifications[key])
              console.log("key : ", key)
              }}
              >
                {/* On: translate-x-6  */}
